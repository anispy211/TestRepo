...
- (void)viewDidLoad
{
    ...
    [cancelbtn setBackgroundImage:[UIImage imageNamed:@"Btn_02"] forState:UIControlStateNormal];
    [cancelbtn setBackgroundImage:[UIImage imageNamed:@"Btn_02_Highlighted"] forState:UIControlStateHighlighted];
    [cancelbtn setTitle:@"BACK" forState:UIControlStateNormal];
    [cancelbtn.titleLabel setFont:[UIFont fontWithName:@"DIN-Regular" size:FONT_SIZE_FOR_NAVIGATION_BAR_BUTTONS]];
...
- (void)setAllowsMultipleSelection:(BOOL)allowsMultipleSelection
{
    ...
    if (allowsMultipleSelection) {
        ...
        [importBtn setFrame:CGRectMake(0, 0, 60, 30)];
        UIImage * image = [UIImage imageNamed:@"Btn_01"];
        [importBtn setBackgroundImage:image forState:UIControlStateNormal];
        image = [UIImage imageNamed:@"Btn_01_Highlighted"];
        [importBtn setBackgroundImage:image forState:UIControlStateHighlighted];
        [importBtn setTitle:@"IMPORT" forState:UIControlStateNormal];
       // [importBtn.titleLabel setFont:[UIFont systemFontOfSize:13]];
        [importBtn.titleLabel setFont:[UIFont fontWithName:@"DIN-Regular" size:FONT_SIZE_FOR_NAVIGATION_BAR_BUTTONS]];

        [importBtn addTarget:self action:@selector(done:) forControlEvents:UIControlEventTouchUpInside];
        UIBarButtonItem *doneButton = [[UIBarButtonItem alloc] initWithCustomView:importBtn];
...
{
    // Delegate
    if (self.delegate && [self.delegate respondsToSelector:@selector(imagePickerControllerDidCancelQB:)]) {
        [self.delegate imagePickerControllerDidCancelQB:self];
    }
}
...
{
    QBAssetsCollectionViewController *assetsCollectionViewController = [[QBAssetsCollectionViewController alloc] initWithCollectionViewLayout:[SECollectionViewFlowLayout layoutWithAutoSelectRows:NO panToDeselect:YES autoSelectCellsBetweenTouches:NO]];
