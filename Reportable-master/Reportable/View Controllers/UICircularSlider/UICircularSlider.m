...
//  Copyright (c) 2016 Zouhair Mahieddine.
...
- (void)setup {
	...
	self.minimumTrackTintColor = REPORTABLE_ORANGE_COLOR;
	self.maximumTrackTintColor = REPORTABLE_CREAM_COLOR;
...
- (void)drawThumbAtPoint:(CGPoint)sliderButtonCenterPoint inContext:(CGContextRef)context {
    ...
    CGContextSetRGBFillColor(context, 242/255.0, 240/255.0, 237/255.0, 1.0);
    ...
    CGContextSetRGBFillColor(context, 224/255.0, 94/255.0, 0, 1.0);
    
	CGContextMoveToPoint(context, sliderButtonCenterPoint.x, sliderButtonCenterPoint.y);
	CGContextAddArc(context, sliderButtonCenterPoint.x, sliderButtonCenterPoint.y, 7, 0.0, 2*M_PI, NO);
...
- (void)panGestureHappened:(UIPanGestureRecognizer *)panGestureRecognizer {
	...
	switch (panGestureRecognizer.state) {
		case UIGestureRecognizerStateChanged: {
            ...
            [self.delegate seekingInProgressAtValue:self.value];
			break;
		}
        case UIGestureRecognizerStateEnded:
        case UIGestureRecognizerStateCancelled:
                  [self.delegate seekAudioTo:self.value];
            break;
		default:
			break;
	}
